...
import { HikeLinkDarkLogo, HikeLinkWhiteLogo } from "../assets";
...
const Header = () => {
  ...
  return (
    <header
      className={`left-0 right-0 z-[5] w-full ${
        scrollheightSpy < 46 ? "absolute text-white" : "fixed bg-white"
      }`}
    >
      <div
        className={`${
      ...
      <div
        className={`flex justify-between flex-wrap items-center my__space ${
          scrollheightSpy > 45 ? "" : "py-2"
        }`}
      >
        <div className="flex justify-between items-center sm:w-[70%] lg:w-full relative">
          <Link to="/" className="scale-[.98] hover:scale-100 transition-all">
            <img
              ...
              src={scrollheightSpy > 45 ? HikeLinkDarkLogo : HikeLinkWhiteLogo}
              alt="logo"
            />
          </Link>
              ...
              <li
                ...
                className={`py-5 ${
                  headerLink.nestedLinks
                    ...
                    : ""
                } header__link mr-4 hover:text-primary_clr ${
                  pathname === headerLink.link ? "text-primary_clr" : ""
                  ...
                  <ul className="absolute top-full bg-white w-[200px] transition-[display] duration-300">
                    {headerLink.nestedLinks.map((nestedLink, index) => (
                      ...
                      >
                        <Link
                          to={nestedLink.link}
                          className={`text-[#333] block py-[10px] px-[20px] ${
